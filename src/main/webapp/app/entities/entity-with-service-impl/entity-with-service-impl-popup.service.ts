        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.entityWithServiceImplService.find(id).subscribe((entityWithServiceImpl) => {
                    this.ngbModalRef = this.entityWithServiceImplModalRef(component, entityWithServiceImpl);
                    resolve(this.ngbModalRef);
                });
            } else {
                // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
                setTimeout(() => {
                    this.ngbModalRef = this.entityWithServiceImplModalRef(component, new EntityWithServiceImpl());
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
