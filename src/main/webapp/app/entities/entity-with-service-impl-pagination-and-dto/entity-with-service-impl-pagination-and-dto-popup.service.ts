        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.entityWithServiceImplPaginationAndDTOService.find(id).subscribe((entityWithServiceImplPaginationAndDTO) => {
                    this.ngbModalRef = this.entityWithServiceImplPaginationAndDTOModalRef(component, entityWithServiceImplPaginationAndDTO);
                    resolve(this.ngbModalRef);
                });
            } else {
                // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
                setTimeout(() => {
                    this.ngbModalRef = this.entityWithServiceImplPaginationAndDTOModalRef(component, new EntityWithServiceImplPaginationAndDTO());
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
