        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.entityWithServiceClassService.find(id).subscribe((entityWithServiceClass) => {
                    this.ngbModalRef = this.entityWithServiceClassModalRef(component, entityWithServiceClass);
                    resolve(this.ngbModalRef);
                });
            } else {
                // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
                setTimeout(() => {
                    this.ngbModalRef = this.entityWithServiceClassModalRef(component, new EntityWithServiceClass());
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
