        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.entityWithServiceImplAndPaginationService.find(id).subscribe((entityWithServiceImplAndPagination) => {
                    this.ngbModalRef = this.entityWithServiceImplAndPaginationModalRef(component, entityWithServiceImplAndPagination);
                    resolve(this.ngbModalRef);
                });
            } else {
                // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
                setTimeout(() => {
                    this.ngbModalRef = this.entityWithServiceImplAndPaginationModalRef(component, new EntityWithServiceImplAndPagination());
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
